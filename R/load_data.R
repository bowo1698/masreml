...
#' n_qtl=20) and \code{size = "small"} (default; n=100, p=50, n_qtl=5) that
#' runs during \code{R CMD check --examples}.
...
#'     and \code{\link{build_G_snp}()}. Dimensions: 200 x 500 (large) /
#'     100 x 50 (small).}
...
#'     consumers (e.g. \pkg{CMplot}). Dimensions: 500 x 3 (large) /
#'     50 x 3 (small).}
...
#'     consume \code{d$map_mh} without translation. Dimensions: 250 x 5
#'     (large) / 25 x 5 (small).}
...
#' \describe{
#'   \item{Scenario A}{Train on gen-1, predict gen-2. Build the full G
#'     across gen-1 + gen-2 (passing the gen-1 slice as \code{ref_W} /
#'     \code{ref_mh} so allele frequencies come from training only),
#'     fit \code{\link{masreml}()} on the gen-1 block, then call
#'     \code{\link{predict.masreml}()} with \code{train_ids} = gen-1,
#'     \code{test_ids} = gen-2 ids.}
...
#'     and \code{ref_W} / \code{ref_mh} comes from the gen-1 + gen-2
#'     training slice.}
#' }
#' Sub-fields under \code{d$multigen}:
#' \describe{
#'   \item{\code{gen1}}{List(\code{snp}, \code{mh}, \code{allele_freq},
#'     \code{pheno}, \code{sire_of}, \code{dam_of}). \code{pheno}
#'     carries both \code{y_cont_qtl_snp} and \code{y_cont_qtl_mh}
#'     plus matching \code{tbv_qtl_*} columns.}
#'   \item{\code{gen2_snp}, \code{gen2_mh}}{Gen-2 cohort lists; same
#'     shape as \code{gen1} but \code{pheno} only carries the
#'     architecture-matching trait + \code{tbv_qtl_*_true} ground
#'     truth.}
#'   \item{\code{gen3_snp}, \code{gen3_mh}}{Gen-3 cohort lists, same
#'     pattern as gen-2; \code{allele_freq} is the gen-1 + gen-2
#'     combined frequency table.}
#'   \item{\code{reference_structure_gen1},
#'         \code{reference_structure_gen1_gen2_snp},
#'         \code{reference_structure_gen1_gen2_mh}}{Full
#'     \code{construct_wah_matrix()} return values (from \pkg{masbayes})
#'     for cross-package interoperability; \pkg{masreml}'s
#'     \code{build_G_mh()} does not need them.}
#'   \item{\code{qtl}}{QTL info: \code{mh_idx}, \code{effects_mh},
#'     \code{beta_mh}, and SNP centring-mean vectors for Scenario A / B.}
#'   \item{\code{pedigree}}{Combined pedigree across founders + gen-1 +
#'     two gen-2 cohorts + two gen-3 cohorts. Passes directly to
#'     \code{\link{build_A_ped}()}.}
#'   \item{\code{ocs}}{Diagnostics for all four OCS calls.}
#'   \item{\code{meta}}{Per-cohort sizes, realised \eqn{h^2} per
#'     generation, target \eqn{\Delta F}, seed, and package versions.}
#' }
#'
#'
#' @examples
