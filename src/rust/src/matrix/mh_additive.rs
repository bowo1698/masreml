...
pub fn build_g_mh_add_internal(
    ...
    for locus in 0..n_loci {
        ...
        // Accumulate W_l * W_l' (no division by k yet)
        let wwt = w_l.dot(&w_l.t());
        agh_num += &wwt;

        // Accumulate tr(W_l * W_l') for k computation
...
pub fn build_g_mh_add(
    ...
    let gmat = build_g_mh_add_internal(&h1, &h2, &n_alleles_usize)
        .map_err(|e| Error::from(e.to_string()))?;
